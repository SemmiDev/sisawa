...
(function () {
    ...
    WordSearch.prototype.drawmatrix = function () {
        for (var row = 0; row < this.settings.gridSize; row++) {
            ...
            const fontSize = (screenW - 200) / 10;

            for (var col = 0; col < this.settings.gridSize; col++) {
                var cvEl = document.createElement("canvas");
                cvEl.setAttribute("class", "ws-col");
                cvEl.setAttribute("width", screenW > 520 ? 40 : fontSize + 2);
                cvEl.setAttribute("height", screenW > 520 ? 40 : fontSize + 2);
                ...
                ctx.font = `400 ${screenW < 520 ? fontSize : 30}px Calibri`;
    ...
    WordSearch.prototype.lookup = function (selected) {
        ...
        if (
            ...
            for (var i = 0; i < wordListItems.length; i++) {
                if (
                    words[0] ==
                    removeDiacritics(wordListItems[i].innerHTML.toUpperCase())
                ) {
                    if (
    ...
    WordSearch.prototype.gameOver = function () {
        ...
        overlay.innerHTML =
            ...
            "<h2>Rampung!</h2>" +
            "<p>Kabeh aksara wis ditemukake!</p>" +
            "</div>" +
            "</div>";
    };
...
function searchLanguage(firstLetter) {
    ...
    var codeLetter = [65, 90]; // Default to Latin alphabet range
    if (codefirstLetter >= 65 && codefirstLetter <= 90) {
        // Latin A -> Z
        return codeLetter;
    }
    console.log("Non-Latin letter detected: " + firstLetter + ":" + codefirstLetter);
    return codeLetter;
}

var daftarKata = [];
$(".ws-words li").each(function (index, li) {
    daftarKata.push($(li).text());
});
